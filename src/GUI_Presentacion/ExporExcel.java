...
public class ExporExcel {
    ...
    public ExporExcel(List<JTable> tab, String nombre, int num, File ar) throws Exception {
        this.archi = ar;
        this.tabla = tab;
        this.nomhoja = nombre;
        this.hoja = num;
    ...
    private void createFilas(WritableSheet sheet, int number_columna, int filas, String name_filas) throws WriteException {
        try {
            ...
            addFilas(sheet, number_columna, filas + 4, name_filas, fomato_fila);//para no imprimir el id
        } catch (Exception e) {
            JOptionPane.showMessageDialog(null, e.getMessage());
        }
    }
...
//        ImprimirImagen(sheet);

        WritableCell titleCell = new Label(1/*columna x*/, 0/*celda y*/, /*texto*/ dato0, format/*formato*/);
        sheet.addCell(titleCell);
        sheet.mergeCells(1/*celda inicio*/, 0/*x*/, table.getColumnCount() - 3/*celda termina*/, 0/*y*/);
        WritableCell titleCell1 = new Label(1, 1, dato1, format);
        sheet.addCell(titleCell1);
        sheet.mergeCells(1, 1, table.getColumnCount() - 3, 1);
        WritableCell titleCell2 = new Label(1, 2, dato2, format);
        sheet.addCell(titleCell2);
        sheet.mergeCells(1, 2, table.getColumnCount() - 3, 2);
        WritableCell titleCell3 = new Label(1, 3, dato3, format);
        sheet.addCell(titleCell3);
        sheet.mergeCells(1, 3, table.getColumnCount() - 2, 3);

        WritableCell titleCell4 = new Label(table.getColumnCount() - 2, 0, dato4, format);
        sheet.addCell(titleCell4);
//        sheet.mergeCells(table.getColumnCount()-4, 0, table.getColumnCount()-3, 0);

        WritableCell titleCell5 = new Label(table.getColumnCount() - 2, 1, dato5, format);
        sheet.addCell(titleCell5);
        sheet.mergeCells(table.getColumnCount() - 2, 1, table.getColumnCount() - 2, 2);
    }
...
//          File imageFile = new File("C:\\Sistema\\ico\\Pescadero128x128.jpg");
        WritableImage imgobj = new WritableImage(0.30/*celda x*/, 0.50/*celda y*/, 0.30/*ancho*/, 3.0/*alto*/, imageFile);
